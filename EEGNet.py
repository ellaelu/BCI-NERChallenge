...
train_labels = pd.read_csv('examples\Data\TrainLabels.csv')

X_train_valid = np.load('examples\Data\X_train_bwbs.npy')
X_train_valid = np.reshape(X_train_valid, (16*340, 56,260))

y_train_valid = train_labels['Prediction'].values

X_test = np.load('examples\Data\X_test_bwbs.npy')
X_test = np.reshape(X_test,(3400,56,260))

y_test = np.reshape(pd.read_csv('examples\Data\TestLabels.csv', header=None).values, 3400)
...
filepath = 'best_weights_bwbs.hdf5'
